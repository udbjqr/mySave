...
export default React.createClass({
    ...
    getInitialState:function(){
       ...
       var vipstrData= JSON.parse(JSON.stringify(sessionStorage.getItem("vip_strData")));
       console.log(vipstrData);
        console.log("vipstrData:"+JSON.parse(vipstrData).position)
        return {
            isError:{},                 /*错误对象初始值*/
            chainCustomerId:[],         /*门店客户*/
            keyval:true,
            customers:[],        /*门店集合*/
            customerid:JSON.parse(vipstrData).customer_id, //下拉选择的门店
            buyer:"",//门店联系人
            customerName:"",//门店名称
            areaName:"",//区域名称
            personInChargeName:"",//主管
            attributesName:"", //门店类型
            phone_number:JSON.parse(vipstrData).phone_number,//手机
            birthday:JSON.parse(vipstrData).birthday,//生日
            email:JSON.parse(vipstrData).email,//邮箱
            address:JSON.parse(vipstrData).address,//地址
            real_name:JSON.parse(vipstrData).real_name, //客户姓名
            position:JSON.parse(vipstrData).position,//客户职位
            customer_type:JSON.parse(vipstrData).customer_type,//客户类型
            vipId:JSON.parse(vipstrData).id,//客户类型
        }
    },//之前
       ...
       if(this.state.address==""){
        ...
        return false;
            ...
            this.ajaxFn(object,function(data){
                message.success("修改vip客户信息成功！");
                window.location.href = 'index.html#/vipclientinfo';
    ...
    customerEvent(){
          /*请求客户信息数据*/
        this.ajaxFn({"controlType":"load","id":this.state.customerid},function (data) {
            this.setState({
 ...
 return (
   ...
   <div className="row addinformation mt30">
    ...
    render:function(){
        return (
            <div>
            <div className="mb-title" >修改vip客户</div>
         
            {this.showcustominfo()}
              <div className="row addinformation mt30">
                    <div className="col-xs-12 col-sm-12 col-md-6 col-lg-4">
                        <InputBox tle="客户姓名" typeName="real_name" even={this.option} defaultValue={this.state.real_name}/>
                    </div>
                    
                    <div className="col-xs-12 col-sm-12 col-md-6 col-lg-4">
                        <InputBox tle="客户职位" typeName="position" even={this.option} defaultValue={this.state.position}/>
                    </div>
                    <div className="col-lg-4">
                     <MultiSelect tle="客户类型"  even={this.customeroption}  placeholder="请选择门店" typeName="customer_type" defaultValue={this.state.customer_type=="1"?"一星":"二星"}  options={[{"name":"一星","id":1},{"name":"二星","id":2}]} divStyle={{width:"100%",minWidth:"100px"}} selectStyle={{ "width": "170px" }} />
              ...
              <div className="row addinformation mt30">
                    <div className="col-xs-12 col-sm-12 col-md-6 col-lg-4">
                        <InputBox tle="手机号码" typeName="phone_number" even={this.phoneOption} defaultValue={this.state.phone_number} />
                    </div>
                    
                    <div className="col-xs-12 col-sm-12 col-md-6 col-lg-4">
                            
                        <DateRange size="large" showTime searchTime={this.searchTime} defaultValue={this.state.birthday} />
                        {/*<InputBox tle="客户生日" typeName="birthday" even={this.option}/>*/}
                    </div>
                    <div className="col-lg-4">
                     <InputBox tle="邮箱" typeName="email" even={this.option} defaultValue={this.state.email}/>
                    </div>
                </div>
                <div className="row addinformation mt30">
                    <div className="col-lg-8">
                    <RegionInput tle="地址" typeName="address" even={this.option} defaultValue={this.state.address} />
                    </div>
                </div>
        ...
        return (
            <div className="form-group">
                ...
                <div className={"col-lg-8 "+(this.props.isStars ? this.props.isStars : "")}>
                    <input type="text"  className="form-control h28" onChange={this.handleChange} defaultValue={this.props.defaultValue}/>
                </div>
            </div>
        );
    }
