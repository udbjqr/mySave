...
export default React.createClass({
    ...
    pageNumber(page) {
        this.ajaxFn({ "controlType": "query", "pageSize": this.state.showLine, "pageNumber": page, "roleName": this.state.memberName }, function (data) {
    ...
    searchTime: function (sd, ed) {
        if (sd != "" && ed != "") {
            ...
            this.ajaxFn({ "controlType": "query", "pageSize": this.state.showLine, "pageNumber": 1, "startDate": sd, "endDate": ed }, function (data) {
                this.setState({
                    dataTab: data.map.list,
                    dataLen: data.map.count,
                    pageNum: 1
    ...
    nameSearch: function (e) {
        ...
        if ((e && e.keyCode == 13) || e.type == "click") {
            ...
            this.ajaxFn({ "controlType": "query", "roleName": memberName, "pageSize": 20, "pageNumber": 1 }, function (data) {
                if (data.map.data == "0") {
                    message.warning('没有相关角色！');
                }
                this.setState({
                    dataLen: data.map.count,
                    dataTab: data.map.list,
    ...
    render: function () {
        return (
            <QueueAnim>
                <div className="mb-title" key="1">角色管理</div>
                <div className="operation" id="operation" key="2">
                    ...
                    <a href="#newRole" className="btn btn-add"><span className="icon-add"></span>新增</a>
                    <div className="form-group fr mem-search">
                        <input type="text" className="fl pl15" placeholder="搜索角色名称" ref="memberName" onKeyUp={this.nameSearch} />
                ...
                <MemberTabel
                    ...
                    titleName={["checkbox", "序号", "角色名称", "角色说明", "创建时间", "操作"]}
                    dataKey={["index", "group_name", "remark", "create_time"]}
                    controlArr={[{ conId: 2, callback: this.roleEdit, conUrl: "modifyRole", conClass: "cleflo" }]}
                    />
