$axure.internal(function($ax) {
    ...
    //        if($ax.style.IsWidgetDisabled(id)) result "disabled";
    //        if($ax.style.IsWidgetSelected(id)) result "selected";
    //        if($ax.event.mouseOverObjectId == id) result "mouseOver";
    //        if($ax.event.mouseDownObjectId == id) result "mouseDown";
    ...
    //    result $.grep(
    //        $('#' + id).children().map(function (i, obj) { result obj.id; }), // all the child ids
    //        function (item) { result item.indexOf(id) < 0; })[0]; // that are not similar to the parent
    //}
    ...
    var _getColorFromFill = function(fill) {
        ...
        //result '#' + fillString.substring(fillString.length - 6);
        var val = fill.color;
        var color = {};
        color.b = val % 256;
