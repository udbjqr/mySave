...
class ConnectionPool {
	...
	private synchronized void clear() {
		LinkedList<MyConnection> needDel = new LinkedList<>();
		myConnections.stream().filter(connection -> connection.busy
						&& (System.currentTimeMillis() - connection.beginUserTime > connectionTimeOut)).forEach(connection -> {
			try {
				connection.close();
			} catch (SQLException e) {
				logger.error("关闭连接出现异常。", e);
			}
			needDel.add(connection);
		});

		for (MyConnection connection : needDel) {
	...
	private MyConnection handleIdle(MyConnection connection) {
		if (System.currentTimeMillis() - connection.beginUserTime > 60 * 1000 * 5) {
