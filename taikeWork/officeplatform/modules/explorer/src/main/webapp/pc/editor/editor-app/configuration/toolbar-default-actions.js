...
KISBPM.TOOLBAR = {
    ACTIONS: {
        ...
        closeEditor: function(services) {
        	window.location.href = "../#/processPage";
        },
...
var SaveModelCtrl = [ '$rootScope', '$scope', '$http', '$route', '$location',
    ...
    $scope.saveDialog.name = json.properties.process_id;
    $scope.saveDialog.description = json.properties.documentation;
    ...
    $scope.save = function (successCallback) {
        ...
        // Indicator spinner image 指示器旋转图像
        $scope.status = {
        	loading: true
        };
        ...
        // Get the serialized svg image source 获取序列化的SVG图像源
        ...
        // Remove all forced stripable elements 删除所有强制可剥性元素
        var stripOutArray = jQuery(svgClone).find(".stripable-element-force");
        for (var i = stripOutArray.length - 1; i >= 0; i--) {
            stripOutArray[i].remove();
        }

        // Parse dom to string 解析DOM的字符串
        var svgDOM = DataManager.serialize(svgClone);

        var params = {
            json_xml: json,
            svg_xml: svgDOM,
            model_name: $scope.saveDialog.name,
        ...
        $http({    method: 'POST',
            ...
            url: KISBPM.URL.putModel(modelMetaData.model_id)})
