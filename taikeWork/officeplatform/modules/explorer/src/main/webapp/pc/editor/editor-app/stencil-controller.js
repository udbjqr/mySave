        ...
        $scope.editorFactory.promise.then(function () {
            ...
            $http({method: 'GET', url: KISBPM.URL.getStencilSet()}).success(function (data, status, headers, config) {
                ...
                for (var stencilIndex = 0; stencilIndex < data.stencils.length; stencilIndex++) 
                {
                    ...
                    // Construct the stencil item 构建模板项
                    var stencilItem = {'id': data.stencils[stencilIndex].id,
                        ...
                        'gaoji':data.stencils[stencilIndex].gaoji,
                        'removed': removed,
                        'customIcon': false,
                        'canConnect': false,
                        'canConnectTo': false,
                    ...
                    if (currentGroup) {
	                    // Add the stencil item to the correct group 模板项添加到正确的组
                        ...
                        // It's a root stencil element 这是一个根元素模板
                        if (!removed) {
                ...
                // remove quick menu items which are not available anymore due to custom pallette 删除快捷菜单项是由于自定义调色板不再可用
            ...
            //         $scope.thisid.push($scope.editor.getCanvas().getChildNodes(true)[i].id)
            //     }
...
//         $scope.__count = 0;
//      }
//      console.log($scope.__count);
//  })
            /*
             * Listen to selection change events: show properties 听选择更改事件:显示属性
             */
            //改变事件
            $scope.editor.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, function (event) {
                ...
                if (shapes && shapes.length > 0) {
                    ...
                    // Store previous selection 存储以前的选择
                    $scope.previousSelectedShape = $scope.selectedShape;
                    // console.log(selectedShape.children);
                    // console.log($scope.editor.getCanvas().getChildNodes(true));
                    ...
                    if ($scope.selectedShape !== undefined && $scope.selectedShape.getId() === selectedShape.getId()) {
                        ...
                        } else if(event.forceSelectionRefresh && event.forceSelectionRefresh === true) {
                        } else {
                    ...
                    for (var i = 0; i < properties.length; i++) {
                        ...
                        if (propertyConfig === undefined || propertyConfig === null) {
                            ...
                            //currentProperty是选择到的下面的名称详情
                             console.log(currentProperty);
                            if ((currentProperty.type === 'complex' || currentProperty.type === 'multiplecomplex' || currentProperty.type ==='formkeydefinitionpackage') && currentProperty.value && currentProperty.value.length > 0) {
                                try {
        ...
        /* Click handler for clicking a property 单击处理程序点击属性 */
        ...
        /* Helper method to retrieve the template url for a property 检索url属性的模板 */
        ...
        /* 所有子控制器（用于属性控制器）更新内部Oryx模型的方法   (更新属性) */
        $scope.updatePropertyInModel = function (property, shapeId) {
            ...
            if (newValue != oldValue) {
                ...
                // Fire event to all who is interested 事件给所有监听的人
        ...
        $scope.myUpdatePropertyInModel = function (key,newValue, shapeId) {
